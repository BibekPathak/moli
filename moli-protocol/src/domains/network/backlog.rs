...
fn emit_staged_subresource_response_started(
    ...
    let timestamp = base_timestamp + ((output.index() + 1) as f64 * 0.000_001);
    let resource_type = request.resource_type().as_cdp_type();
...
#[cfg(test)]
mod tests {
    use crate::domains::network::NetworkPreparedOutputs;

    use super::{NetworkBacklogPreferredRequestId, NetworkBacklogProjectionContext};
