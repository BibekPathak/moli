...
- Playwright upstream derived samples for route request metadata, `page.route()` precedence over `context.route()`, context fallback, `times`, fallback chaining, terminal fulfill/abort routing, `route.fulfill()` cookies/headers, explicit `page.pdf()` unsupported errors, page/main-frame/browser `CDPSession.send()`, session Network event delivery, unknown command errors, and post-detach command rejection.
...
- Screenshot/layout/emulation: full-page/clip/node screenshots, PDF generation, richer device-scale behavior, geolocation, permissions, color scheme, media emulation, and more accurate layout metrics. Raw CDP and explicit Puppeteer current-viewport PNG capture are supported. The Chrome `chrome://inspect` command menu is a separate DevTools-frontend concern and does not advertise Moli's screenshot capability.
