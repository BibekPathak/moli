...
fn main() {
  ...
  // Watch the directory, not a hand-maintained file list. Cargo then reruns
  // this script when a local bridge source is added or removed as well as
  // when an existing source/header changes. The same discovered `_ext.cc`
  // set is passed to `cc::Build` below, so dependency tracking and compiler
  ...
  if env_bool("V8_FROM_SOURCE") {
    ...
    // Cargo publish and incremental local source builds should not leave pyc
    // files under the restored upstream rusty_v8 tree.
    unsafe {
...
fn moli_v8_ext_sources() -> Vec<PathBuf> {
  let mut sources = fs::read_dir("src")
    ...
    .map(|entry| entry.expect("vendored V8 src entry must be readable").path())
    .filter(|path| {
      path
        .file_name()
...
This Moli checkout currently has the slim prebuilt-only vendor tree. \
Missing required path(s): {}. Restore the full rusty_v8/V8 source tree before \
using V8_FROM_SOURCE=1, or provide a matching custom RUSTY_V8_ARCHIVE for \
prebuilt builds.",
      missing.join(", ")
    );
  }
...
fn download_file(url: &str, filename: &Path) {
  ...
  // Prefer curl for the slim vendored tree. Keep the Python helper as an
  // optional fallback when the full upstream layout is restored.
  let status = match status {
    ...
    _ => {
      ...
      match curl_status {
        ...
        _ if Path::new("./tools/download_file.py").exists() => {
          println!("curl downloader failed, trying with Python helper...");
          Command::new(python())
        ...
        _ => {
          panic!(
            "failed to download {url}: neither Deno nor curl succeeded, and \
tools/download_file.py is unavailable in the slim vendored tree"
          );
        }
      }
    }
